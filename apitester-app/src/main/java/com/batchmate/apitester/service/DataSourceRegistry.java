...
/**
 * Pools one HikariCP DataSource per saved DB connection and binds it into the Camel
 * registry as "ds-<connectionId>" — the exact bean name workflow-plugin-db's
 * "dbexecute" converter addresses via jdbc:ds-<id>. Copied from workflow-app.
 */
@Service
@DependsOn("requestConverterRegistry")  // ensures plugin JARs (with JDBC drivers) are loaded first
public class DataSourceRegistry {
    ...
    // ── JDBC URL builder (mirrors workflow-plugin-db's DbPlugin URL logic) ────

    private static HikariConfig buildConfig(JsonNode c) {
        HikariConfig cfg = new HikariConfig();
        cfg.setPoolName("apitester-ds-" + c.path("id").asText());
