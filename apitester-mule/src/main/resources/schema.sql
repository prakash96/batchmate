...
-- '*' for "any method"; "path" is a template like /users/{id}/orders — a {segment} matches any
-- literal value in that position, matched against whatever comes in under this app's own
-- /mock/** listener. response_body is stored and served as raw text exactly as entered (JSON, XML,
-- plain text, whatever content_type claims it is) — never re-parsed/re-serialized.
CREATE TABLE mock_endpoints (
    id                    VARCHAR2(64) PRIMARY KEY,
    name                  VARCHAR2(255) NOT NULL,
    method                VARCHAR2(16) DEFAULT '*' NOT NULL,
    path                  VARCHAR2(500) NOT NULL,
    status_code           NUMBER(5) DEFAULT 200 NOT NULL,
    content_type          VARCHAR2(255) DEFAULT 'application/json' NOT NULL,
    response_headers_json CLOB DEFAULT '{}' NOT NULL,
    response_body         CLOB,
