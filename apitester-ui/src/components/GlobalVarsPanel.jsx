...
export default function GlobalVarsPanel({ onClose }) {
    const { globalVariables, setGlobalVariables } = useGlobalVarsStore();
    const [rows, setRows] = useState(Object.entries(globalVariables).map(([name, value]) => ({ name, value })));
        ...
        <div className="at-overlay" style={overlayStyle}>
            <div className="at-modal" style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: C.radius, boxShadow: C.shadowLg, width: 460, padding: 20 }}>
                ...
                <div style={{ fontSize: 11, color: C.textFaint, marginBottom: 12 }}>
                    Injected as <code style={{ color: C.textDim }}>vars.x</code> on every request run, before collection variables and pre-request steps override them.
                </div>
                {rows.map((row, i) => (
